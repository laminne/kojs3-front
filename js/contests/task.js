...
window.onload = async () => {
    ...
    try {
        ...
        Lib.getElementById("contest-title").innerHTML = `<a href="show.html?id=${contest.id}">${contest.title}</a>`;
        Lib.getElementById("task-name").innerText = res.title;
        Lib.getElementById("task-area").innerHTML = res.description;
        Lib.getElementById("task-constrains").innerText = `実行時間制限: ${res.time_limit}sec / メモリ制限: ${res.memory_limit}`;
    }
    catch (e) {
        console.error(e);
