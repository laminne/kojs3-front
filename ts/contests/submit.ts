...
window.onload = async () => {
  ...
  Lib.getElementById(
    "contest-name"
  ).innerHTML = `<a href="show.html?id=${contestData.id}">${contestData.title}</a>`;
  ...
  Lib.getElementById("submit").addEventListener("click", async () => {
    const selectedTask = Lib.getElementById("task-id") as HTMLSelectElement;
    const selectedLanguageType = Lib.getElementById(
      "lang-type"
    ) as HTMLSelectElement;
    const code = Lib.getElementById("code-area") as HTMLTextAreaElement;
    ...
    WSConnection.onmessage = async (e) => {
      ...
      if (
        message.type === "JudgementStateUpdated" &&
        message.uid === Lib.parseJWTToken(token)
      ) {
        const judgement = await api.get(
          `/contests/${id}/submissions/${message.sid}`,
          token
        );
...
function updateJudgementState(d: judgementResponse, res: any) {
  ...
  submissionsContainer.innerHTML = `
    ...
    <td class="border px-4 py-2">${
      res[(Lib.getElementById("task-id") as HTMLSelectElement).value].title
    }</td>
    <td class="border px-4 py-2">${
      (Lib.getElementById("lang-type") as HTMLSelectElement).value
    }</td>
    <td class="border px-4 py-2"><span class="${applyStateColor(
      d.state
    )}  text-white rounded px-4 py-2">${d.state}</span></td>
    <td class="border px-4 py-2">終了</td>
    <td class="border px-4 py-2">終了</td>
  `;
}

function applyStateColor(state: string) {
  switch (state) {
    case "AC":
